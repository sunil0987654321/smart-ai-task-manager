...
import { Sparkles, Loader, CheckSquare, Search } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const [sortOption, setSortOption] = useState(() => localStorage.getItem('task_sortOption') || 'Deadline');
  ...
  }, [tasksArray, searchTerm, filterPriority, filterStatus]);
  ...
  useEffect(() => {
    setVisibleCount(10);
  }, [searchTerm, filterPriority, filterStatus, sortOption]);
  ...
  const renderSkeletons = () => (
    <div className="space-y-4 w-full">
      {[1, 2, 3].map((n) => (
        <div key={n} className="bg-white dark:bg-slate-900/60 p-5 rounded-3xl shadow-sm border border-gray-100 dark:border-white/5 flex items-start space-x-4 relative overflow-hidden">
          <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-black/5 dark:via-white/10 to-transparent animate-shimmer"></div>
          <div className="w-6 h-6 bg-gray-200 dark:bg-slate-700/50 rounded-full mt-1 shrink-0"></div>
          <div className="flex-1 space-y-3 py-1">
            <div className="h-5 bg-gray-200 dark:bg-slate-700/50 rounded w-1/3"></div>
            <div className="h-3 bg-gray-200 dark:bg-slate-700/50 rounded w-2/3 mt-2"></div>
            <div className="flex space-x-2 mt-4 pt-2">
              <div className="h-4 bg-gray-200 dark:bg-slate-700/50 rounded-full w-16"></div>
              <div className="h-4 bg-gray-200 dark:bg-slate-700/50 rounded-full w-24"></div>
            </div>
          </div>
        </div>
      ))}
          ...
          <div className="flex space-x-2 mb-6 overflow-x-auto pb-2 scrollbar-hide">
            {['All', 'Todo', 'InProgress', 'Completed'].map(status => (
              <button
                ...
                className={`flex items-center px-4 py-2 rounded-xl text-sm font-medium transition-all duration-300 whitespace-nowrap focus:outline-none focus:ring-2 focus:ring-indigo-500 ${filterStatus === status
              ...
              >
                ...
                <span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-bold ${filterStatus === status ? 'bg-indigo-400/50 text-white' : 'bg-gray-100 dark:bg-white/5 text-gray-500 dark:text-slate-500'
                  }`}>
                  {statusCounts[status]}
                </span>
              </button>
          ...
          <div className="flex-grow">
            {isLoading && tasksArray.length === 0 ? (
              ...
              <motion.div
                ...
                className="flex flex-col items-center justify-center h-64 text-gray-500 dark:text-slate-400 text-center"
              >
                {isError ? (
                  <div className="text-center">
                    <p className="text-lg text-red-500 dark:text-red-400 mb-4 font-medium">Failed to load tasks.</p>
                    <button
                      onClick={() => dispatch(getTasks())}
                      className="px-6 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition shadow-lg shadow-indigo-200/50 dark:shadow-indigo-900/20 active:scale-95 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900"
                    >
                      Retry Now
                    </button>
                  </div>
                  ...
                  <>
                    <div className="bg-indigo-50 dark:bg-indigo-500/10 p-5 rounded-full mb-4 shadow-inner">
                      {searchTerm ? (
                        <Search size={36} className="text-indigo-400 dark:text-indigo-500" />
                      ) : (
                        <CheckSquare size={36} className="text-indigo-400 dark:text-indigo-500" />
                      )}
                    </div>
                    <p className="text-xl font-semibold text-gray-700 dark:text-slate-200">
                      {searchTerm ? 'No matching tasks found' : filterStatus !== 'All' ? `No ${filterStatus.toLowerCase()} tasks` : 'No tasks yet'}
                    </p>
                    {tasksArray.length === 0 ? (
                      <p className="text-sm mt-2 max-w-sm text-gray-500 dark:text-slate-400">Your task list is empty. Add a new task on the left or use AI to generate some ideas!</p>
                    ) : (
                      <p className="text-sm mt-2 max-w-sm text-gray-500 dark:text-slate-400">Try adjusting your filters or search term to find what you're looking for.</p>
                    )}
                  </>
                )}
              </motion.div>
